// configuracoes.js (atualizado com tabelas para históricos e exibição de avatar - removido o carregamento de avatar/nome pois agora é comum)
  ...
  try {
    ...
    if (requisicoes.length === 0) {
      ...
      requisicoes.forEach((req) => {
        ...
        const tr = document.createElement("tr");
        tr.innerHTML = `<td data-label="ID do Item">${
          req.id_item
        }</td><td data-label="Descrição">${
          req.descricao
        }</td><td data-label="Status"><span class="status-cell ${statusClass}">${
          req.status || "Pendente"
        }</span></td>`;
        tbody.appendChild(tr);
      });
